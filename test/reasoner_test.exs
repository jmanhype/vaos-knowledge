defmodule Vaos.Knowledge.ReasonerTest do
  ...
  test "subClassOf transitivity", %{state: state} do
  ...
  test "owl:inverseOf", %{state: state} do
    {:ok, state} = ETS.assert_many(state, [
  ...
  test "owl:TransitiveProperty", %{state: state} do
    {:ok, state} = ETS.assert_many(state, [
  ...
  test "owl:SymmetricProperty", %{state: state} do
    {:ok, state} = ETS.assert_many(state, [
  ...
  test "fixpoint with no new inferences returns 0 rounds", %{state: state} do
    {:ok, state} = ETS.assert(state, {"a", "b", "c"})
    {:ok, _state, rounds} = Reasoner.materialize(ETS, state)
    assert rounds == 0
  end

  test "multiple rounds of reasoning", %{state: state} do
    {:ok, state} = ETS.assert_many(state, [
      {"A", "rdfs:subClassOf", "B"},
      {"B", "rdfs:subClassOf", "C"},
      {"C", "rdfs:subClassOf", "D"}
